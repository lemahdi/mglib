...
MG_ZeroCurve* BuildZC(const MG_Date& aAsOf)
{
	...
	vector<double> vZeroRates(10);  /* Zero rates in percentage (e.g., 2.0 = 2%) */

	vMat[0] = 0.5; vMat[1] = 1; vMat[2] = 2; vMat[3] = 3; vMat[4] = 4;
	vMat[5] = 5; vMat[6] = 6; vMat[7] = 7; vMat[8] = 8; vMat[9] = 10;

	/* Flat 2% zero rate curve: DiscountFactor(T) = exp(-0.02 * T) */
	for(size_t i=0; i<10; ++i)
		vZeroRates[i] = 2.;

	return new MG_ZeroCurve(aAsOf, vMat, vZeroRates, "EUR", "EURIB", interpoltypeLinear);
}
